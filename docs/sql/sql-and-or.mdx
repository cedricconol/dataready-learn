...
Wait - order 9 has status `pending`. Let's re-check: with `AND`, both conditions must pass. Order 9 (`pending`, `999.00`) does **not** pass the `status = 'completed'` check, so it is excluded.
