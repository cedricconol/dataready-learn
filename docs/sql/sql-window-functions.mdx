...
:::tip Prior lessons
Window functions build on [Aggregate Functions](./sql-aggregate-functions), [GROUP BY](./sql-group-by), and [ORDER BY](./sql-order-by). The sample dataset is defined in the [SELECT](./sql-select) lesson.
:::
