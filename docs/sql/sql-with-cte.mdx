...
:::tip Prior lessons
CTEs work best when you already understand [SELECT](./sql-select), [JOIN](./sql-join), [GROUP BY](./sql-group-by), and [Aggregate Functions](./sql-aggregate-functions). The sample dataset is defined in the SELECT lesson.
:::
